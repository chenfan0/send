...
const { normalize, basename, extname, resolve, parse, sep } = require("path");
...
async function send(ctx, path, opts = {}) {
  ...
  // make sure the maxage is a object
  ...
  function getMaxage(extname) {
    const keys = Object.keys(maxage);
    const key = keys.find((key) => path.endsWith(key + extname));
    return key || "default";
  }
  ...
  if (!ctx.response.get("Cache-Control")) {
    const type = getMaxage(encodingExt);
    const directives = [`max-age=${(maxage[type] / 1000) | 0}`];
    if (immutable) {
